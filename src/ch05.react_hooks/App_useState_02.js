...
function App(){
	...
	const [comment, setComment] = useState('재작년차');
	...
	const ChangeTest = (event) => {
		...
		if(target_id === 'color'){
			...
			}else if(target_value === '2022'){
				mycomment = '작년차';
				
			}else if(target_value === '2021'){
				mycomment = '재작년차';
			}
	...
	return(
		<div>
			...
			<p style = {{color}}>
				{color} 색상의 {year}년산 차종 "{model}"<br/>
				{comment}
			</p>
			<br/><br/>
			
			<img src={'/images/'+ image} alt="" width="200" height="200"/>
