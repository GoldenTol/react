...
function App(props){
    ...
    function getProductList(item, index){
        ...
        const trtag = <tr id={item.id} key={index}><td onClick={ClickItem}>{item.name}</td><td onClick={ClickItem} align='right'>{item.price.toLocaleString()} 원</td><td onClick={ClickItem}>{hangul}</td></tr>;
		return trtag ;
    }
