...
// import category_AddContent from './components/category_AddContent';
...
function App() {
  ...
  const getContent = () => {
    ...
    }else if(mode==='get_update'){ /* 수정버튼을 눌렀습니다. */
      const currentRow = getReadContent();
      return <UpdateContent formData={currentRow} onSubmitUpdate={UpdatedData} />;
  ...
  /* 수정된 행정보를 제외한 나머지 모든 상품들만 추려 반환해 줍니다. */
  const ExceptRow = (id) => {
